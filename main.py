...
curr_league_id = st.sidebar.text_input(label='Enter your league ID', value='784428347810816000')
...
with st.spinner('Loading League History...'):
    ...
    reg_szn = get_alltime_rg_matchups(league_ids)
    reg_df = pd.DataFrame(reg_szn)
    #load the post season matchups
    post_szn = get_alltime_po_matchups(league_ids)
    post_df = pd.DataFrame(post_szn)
    #Get All matchups
    matchups_df = get_all_matchups(reg_df, post_df)
    #Create DF with each individual matchup scores
    ind_matchups = get_ind_matchup_scores(matchups_df)
...
highest_scorer, score = get_highest_scorer(reg_df)
